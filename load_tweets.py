...
def insert_tweet(connection,tweet):
    ...
    with connection.begin() as trans:
        ...
        sql=sqlalchemy.sql.text(f'''
            ...
            VALUES
                (:id_tweets, :id_users, :created_at, :in_reply_to_status_id, :in_reply_to_user_id, :quoted_status_id, :retweet_count, :favorite_count, :quote_count, :withheld_copyright, :withheld_in_countries, :source, :text, :country_code, :state_code, :lang, :place_name, ST_GeomFromText(:geo, 4326))
            ON CONFLICT DO NOTHING
            ''')
        connection.execute(sql, {
            'id_tweets': tweet['id'],
            ...
            'geo': None if geo_coords is None else f'{geo_str}({geo_coords})',
            })
        ########################################
        ...
        for mention in mentions:
            ...
            sql=sqlalchemy.sql.text('''
                INSERT INTO users (id_users, name, screen_name)
                VALUES (:id_users, :name, :screen_name)
                ON CONFLICT (id_users) DO NOTHING
                ''')
            connection.execute(sql, {
                'id_users': mention['id'],
